...
def train_xgboost_classifier(
    input_file, output_dir, target_column="FIRE_SIZE_HA", threshold=1.0
):
    """
    ...
    print(
        f"Created binary target '{binary_target}' with threshold {threshold} hectares"
    )
    print(f"Class distribution:")
    print(
        f"  Class 0 (< {threshold} ha): {class_counts[0]} samples ({class_percentages[0]:.2f}%)"
    )
    print(
        f"  Class 1 (>= {threshold} ha): {class_counts[1]} samples ({class_percentages[1]:.2f}%)"
    )

    # If classes are very imbalanced, log a warning
    if min(class_percentages) < 10:
        print(
            f"WARNING: Classes are imbalanced. Consider using class weights or resampling techniques."
        )
    ...
    feature_cols = [
        ...
        if col not in exclude_cols and col != target_column and col != binary_target
    ]
    ...
    if near_constant_features:
        ...
        for col in near_constant_features:
            print(
                f"  - {col}: {df[col].value_counts(normalize=True).max():.2%} most common value"
            )

        # Remove these features
        feature_cols = [
            col for col in feature_cols if col not in near_constant_features
        ]
    ...
    baseline_metrics_df.to_csv(
        os.path.join(output_dir, "baseline_model_metrics.csv"), index=False
    )
    print(
        f"Baseline metrics saved to {os.path.join(output_dir, 'baseline_model_metrics.csv')}"
    )
    ...
    with open(os.path.join(output_dir, "baseline_model_metrics.txt"), "w") as f:
        ...
        # Add default parameters section
        ...
        f.write(f"  min_child_weight: 1\n")  # XGBoost default
        f.write(f"  gamma: 0\n")  # XGBoost default
        f.write(f"  subsample: 1\n")  # XGBoost default
        f.write(f"  colsample_bytree: 1\n")  # XGBoost default
        f.write(f"  reg_alpha: 0\n")  # XGBoost default
        f.write(f"  reg_lambda: 1\n")  # XGBoost default
        f.write(f"  scale_pos_weight: {weight_1 if class_weights else 1.0}\n\n")
        ...
        if hasattr(baseline_model, "best_iteration"):
            f.write(f"\nBest Iteration: {baseline_model.best_iteration}\n")

    print(
        f"Baseline metrics text summary saved to {os.path.join(output_dir, 'baseline_model_metrics.txt')}"
    )
    ...
    feature_importance_df.to_csv(
        os.path.join(output_dir, "baseline_feature_importance.csv"), index=False
    )
    print(
        f"Feature importance saved to {os.path.join(output_dir, 'baseline_feature_importance.csv')}"
    )
    ...
    selected_features = (
        feature_importance_df["Feature"].iloc[:selected_features_count].tolist()
    )

    print(
        f"Selected top {selected_features_count} features (cumulative importance > {importance_threshold})"
    )
    print("Top 5 features:")
    for i, feature in enumerate(selected_features[:5]):
        print(
            f"  {i+1}. {feature}: {feature_importance_df.loc[feature_importance_df['Feature'] == feature, 'Importance'].values[0]:.4f}"
        )
    ...
    # Expanded parameter grid with focus on regularization
    # More efficient parameter grid with focus on regularization
    param_grid = {
        "n_estimators": [100, 200],  # Reduced options
        "max_depth": [3, 5],  # Key parameter for controlling overfitting
        "learning_rate": [0.05, 0.1],  # Only the more common learning rates
        "subsample": [0.8],  # Reduced to single good value
        "colsample_bytree": [0.8],  # Reduced to single good value
        "min_child_weight": [1, 3],  # Important for preventing overfitting
        "gamma": [0, 0.1],  # Minimal options for minimum split loss
        "reg_alpha": [0, 1],  # Basic L1 regularization options
        "reg_lambda": [1, 10],  # Basic L2 regularization options
    }
    ...
    xgb_model = xgb.XGBClassifier(
        ...
        early_stopping_rounds=20,  # Add early stopping here instead of in fit()
    )
    ...
    # Set up callback for early stopping during cross-validation
    ...
    # Fit the grid search
    grid_search.fit(
        X_train_selected, y_train, eval_set=[(X_val_selected, y_val)], verbose=False
    )
    ...
    # Evaluate best model from grid search on validation set
    ...
    train_pred_proba = best_model.predict_proba(X_train_selected)[:, 1]

    # Calculate comprehensive metrics similar to baseline
    tuned_train_accuracy = accuracy_score(y_train, train_pred)
    tuned_val_accuracy = accuracy_score(y_val, val_pred)
    tuned_train_f1 = f1_score(y_train, train_pred)
    tuned_val_f1 = f1_score(y_val, val_pred)
    tuned_train_auc = roc_auc_score(y_train, train_pred_proba)
    tuned_val_auc = roc_auc_score(y_val, val_pred_proba)
    tuned_val_precision = precision_score(y_val, val_pred)
    tuned_val_recall = recall_score(y_val, val_pred)

    # Print metrics
    print(f"\nTuned model metrics:")
    print(
        f"Training Accuracy: {tuned_train_accuracy:.4f}  |  Validation Accuracy: {tuned_val_accuracy:.4f}"
    )
    print(
        f"Training F1 Score: {tuned_train_f1:.4f}  |  Validation F1 Score: {tuned_val_f1:.4f}"
    )
    print(
        f"Training AUC: {tuned_train_auc:.4f}  |  Validation AUC: {tuned_val_auc:.4f}"
    )
    ...
    tuned_metrics_df.to_csv(
        os.path.join(output_dir, "tuned_model_metrics.csv"), index=False
    )
    print(
        f"Tuned model metrics saved to {os.path.join(output_dir, 'tuned_model_metrics.csv')}"
    )
    ...
    print(
        f"Tuned model metrics text summary saved to {os.path.join(output_dir, 'tuned_model_metrics.txt')}"
    )

    # Calculate and print feature importance of best model
    ...
    for i in range(min(5, len(selected_features))):
        ...
        print(
            f"  {selected_features[feature_idx]}: {feature_importance[feature_idx]:.4f}"
        )
    ...
    print(
        f"Training final model on {len(X_train_full)} samples with {len(selected_features)} features"
    )
    ...
    class_report = classification_report(
        y_test, y_test_pred, target_names=[f"<{threshold}ha", f">={threshold}ha"]
    )
    ...
    metrics_df.to_csv(
        os.path.join(output_dir, "model_performance_metrics.csv"), index=False
    )
    print(
        f"Performance metrics saved to {os.path.join(output_dir, 'model_performance_metrics.csv')}"
    )
        ...
        try:
            ...
            for i in range(top_features_to_plot):
                ...
                shap.dependence_plot(
                    feature, shap_values.values, X_test_sample, show=False
                )
                plt.tight_layout()
    ...
    with open(os.path.join(output_dir, "model_training_summary.txt"), "w") as f:
        ...
        f.write(
            f"Classification Task: Predict if fire size is >= {threshold} hectares\n\n"
        )
        ...
        for i, feature in enumerate(selected_features[:5]):
            importance_val = feature_importance_df.loc[
                feature_importance_df["Feature"] == feature, "Importance"
            ].values[0]
    ...
    print(
        f"Model training summary saved to {os.path.join(output_dir, 'model_training_summary.txt')}"
    )
...
if __name__ == "__main__":
    ...
    output_dir = "./models/xgboost/classification_9"
